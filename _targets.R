...
## Try to load locally
try(setwd("~/phd/cheap_subsampling_article/simulation_study/"), silent = TRUE)
try(setwd("~/research/SuperVision/Johan/phd/cheap_subsampling_article/sim_study"),
  silent = TRUE
)
...
if (dir.exists("/projects/biostat01/people/snf991/phd/cheap_subsampling_article/simulation_study")) {
...
b_max <- 500 # number of bootstrap samples
...
list(
  ...
  ## Summaries
  tar_target(subsample_plot, plot_subsample_size(
  ...
  tar_target(bootstrap_plot, plot_bootstrap(
    ...
    b_lower_plot = 1, # dont show the first 1:(b_lower_plot-1) bootstrap samples in the plot with m
...
## Save the results
## tar_load(subsample_plot)
## tar_load(bootstrap_plot)
## tar_load(sample_size_table)
## sample_size_table |> gtsave(filename = "../tables/simulation_study/table_sample_size.tex")
## subsample_plot |> ggsave(filename = "../figures/simulation_study/subsample_plot.pdf", width = 7, height = 7, device = cairo_pdf)
## bootstrap_plot |> ggsave(filename = "../figures/simulation_study/bootstrap_plot.pdf", width = 7, height = 7, device = cairo_pdf)
